...
class QLearningAgent:
    ...
    def update_q(self, state, action, reward, next_state):
        ...
        max_next_q = max([self.q_table.get((next_state_tuple, a), 0.0) for a in possible_actions], default=0.0)
...
def train_agents(episodes=1000, agent1_epsilon=0.1, agent2_epsilon=0.1):
    ...
    # 기존 학습 파일이 있으면 로드, 없으면 새 에이전트 생성
    if os.path.exists("agent1_q.pkl"):
        q_table1 = load_q_table("agent1_q.pkl")
        agent1 = QLearningAgent(epsilon=agent1_epsilon, q_table=q_table1)
    ...
    if os.path.exists("agent2_q.pkl"):
        ...
        agent2 = QLearningAgent(epsilon=agent2_epsilon, q_table=q_table2)
...
if __name__ == "__main__":
    ...
    # 학습된 Q-table 저장 (기존 파일에 추가 학습 결과 반영)
